...
const HTTPError = require('../../src/utils/errors/httpError.js');
...
describe('Auth Controller', () => {
  ...
  describe('checkTokenValidity', () => {
    it('should return 200 status code when token is valid', async () => {
      ...
      const next = jest.fn();
      AuthService.checkTokenValidity = jest.fn().mockReturnValue({'token': 'test'});
      await AuthController.checkTokenValidity(req, res, next);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({ 'status': 200, 'message': 'Token Verified', 'data': {'token': 'test'}});
    });
